...
func TestFinWait2Timeout(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.description, func(t *testing.T) {
			...
			if gotOne, err := conn.Expect(tb.TCP{Flags: tb.Uint8(header.TCPFlagFin | header.TCPFlagAck)}, time.Second); gotOne == nil {
			...
			conn.Send(tb.TCP{Flags: tb.Uint8(header.TCPFlagAck)})
			if tt.linger2 {
				if gotOne, err := conn.Expect(tb.TCP{Flags: tb.Uint8(header.TCPFlagRst)}, time.Second); gotOne == nil {
					t.Fatalf("expected a RST packet within a second but got none: %s", err)
				}
			} else {
				if gotOne, err := conn.Expect(tb.TCP{Flags: tb.Uint8(header.TCPFlagRst)}, 10*time.Second); gotOne != nil {
